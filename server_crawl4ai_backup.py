...
async def get_next_data(url: str, cache_mode: CacheMode = CacheMode.ENABLED):
    ...
    config = CrawlerRunConfig(
        ...
        wait_until="load",  # Just wait for page load, not networkidle (Airbnb never reaches idle)
        delay_before_return_html=5.0,  # Wait 5 seconds for JS to render content
        page_timeout=30000,  # 30 second timeout
        js_code=[
            "await new Promise(resolve => setTimeout(resolve, 2000));",  # Extra 2s wait via JS
        ]
    )
    ...
    if not result.success:
        ...
        raise Exception(f"Failed to scrape data from {url}. Status: {status_code}, Error: {error_msg}")
    ...
    if extraction_failed:
        ...
        # Look for script tags with JSON data (Airbnb uses various formats)
        patterns = [
            ...
            if matches:
                for match in matches[:5]:  # Try first 5 matches
                    try:
                        ...
                        # Check if this looks like Airbnb data
                        if isinstance(json_data, dict) and ('props' in json_data or 'data' in json_data or '@context' in json_data):
                            logger.info(f"Successfully extracted data using HTML fallback (pattern: {pattern[:50]}...)")
                            return json_data.get('props', {}).get('pageProps', json_data)
                    except json.JSONDecodeError:
                        continue
        
        # If JSON extraction failed, try DOM-based extraction as last resort
        logger.warning("JSON extraction failed, attempting DOM-based extraction")
        try:
            from bs4 import BeautifulSoup
            soup = BeautifulSoup(html_content, 'html.parser')
            
            # Look for listing cards in the rendered DOM
            listings = []
            
            # Try various selectors that Airbnb might use
            card_selectors = [
            ...
            for selector in card_selectors:
                ...
                if cards:
                    ...
                    for card in cards[:10]:  # Limit to 10
                        ...
                        if card.get('data-listing-id'):
                            listing['id'] = card.get('data-listing-id')
                        
                        # Try to find name/title
                        title_elem = card.select_one('[data-testid="listing-card-title"], h3, h2, .title')
                        if title_elem:
                            listing['name'] = title_elem.get_text(strip=True)
                        
                        # Try to find price
                        price_elem = card.select_one('[data-testid="price-availability-row"], .price, [aria-label*="price"]')
                        if price_elem:
                        ...
                        if listing.get('id') or listing.get('name'):
                            listings.append(listing)
                    
                    if listings:
                        logger.info(f"Successfully extracted {len(listings)} listings from DOM")
                        # Return in expected format
                        return {'searchResults': {'sections': [{'items': [{'listing': l} for l in listings]}]}}
        ...
        # If everything failed, save HTML for debugging
        logger.error(f"All extraction methods failed for {len(html_content)} chars of HTML")
        ...
        raise Exception("Could not extract Airbnb data using any method. Check debug_airbnb_page.html for full content.")
